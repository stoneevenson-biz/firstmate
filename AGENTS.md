...
1. Run `bin/fm-lock.sh` to acquire the session lock - the helm (it records the harness process PID, which is session-stable).
   **The lock guards mutation, not activation.** If it reports that another live session holds it, you are the observer, and that is a normal state rather than a failure: activate fully, read anything, answer "what is the fleet doing", reason and draft. It says so plainly and exits 0, because for a second instance that is the ordinary outcome, not an error.
   ...
   A drive verb that proceeds on a free or stale helm has TAKEN it, atomically - the seam claims, it does not merely ask - so you never have to acquire it again by hand.
   Which verbs gate, which deliberately do not, and the edge policies (an unidentifiable harness is waved through only when nobody is steering; `fm-bootstrap.sh install` does not gate) are in `docs/scripts.md`, "The helm: which verbs gate on the session lock".
...
- **Merging anywhere that is not this clone's `origin` REFUSES**, and takes a second word to allow: `--allow-non-origin`. An upstream contribution is legitimate and this is how you say so. Being *named* proves nothing was inferred; it does not prove the name was meant, and a URL or a recorded `pr=` line can arrive from anywhere. The affirmation names no repository, so it can never itself be the thing that redirects a merge. An `origin` that cannot be read at all refuses too - a proof that did not run is not a proof that passed.
...
- **Name the target once.** `--repo` or `--remote` or `--project` given twice refuses, and so does a `--repo` and a `--remote` that name different repositories. "The last one wins" is exactly the `gh` behaviour this path exists to stop depending on, so it is never how a disagreement is resolved here.
- Merge options pass through after `--` (`-- --squash --delete-branch`), and passthrough is an **allowlist**: sanctioned long options only. Every short flag is refused as a class, because a short flag can cluster and can carry its value attached (`-Rowner/repo`, `-dR owner/repo`) - which is how a repo flag hid inside one. An option that is genuinely wanted is added to the set in `bin/fm-merge-target-lib.sh`; a refusal here costs a commit, and a permissive passthrough costs a merge.
- **A `GIT_CONFIG*` variable in the environment REFUSES the merge outright**, on the rail `env/git-config-injected`, naming what it found. That family substitutes git's *configuration* rather than its repository, and a remote URL is configuration: `url.<attacker>.insteadOf` rewrites what `remote get-url` answers, so the resolved target and the origin proof that checks it read the same substituted value and agree with each other perfectly. Every read on this path now ignores the family (swept by prefix - its indices are unbounded, so no list can cover it) and pins `HOME` and `XDG_CONFIG_HOME` at a path that cannot hold anything, so nothing can land in the wrong repository either way. The `HOME` pin is the load-bearing half and is not interchangeable with disabling git's global lookup: that only stops git going looking, while a `~` inside the repository's own `include.path` still expands through a hostile `HOME` - which is exactly how the first version of this fix was defeated. The refusal is the second half: nothing sets `GIT_CONFIG_COUNT` by accident, so its presence in a merge path is the finding, not an inconvenience. This is deliberately unlike `GH_REPO`, `GIT_DIR` and `HOME`, which are pinned or scrubbed but never refused on because ordinary tooling sets them. What this does NOT cover is an environment that replaces the git BINARY - a fake `git` on `PATH` still redirects a merge, and no check inside the process can stop that, because the same environment could replace `bin/fm-merge-pr.sh`. That boundary is gated, not just written down; do not read the rail above as a promise that a merge is safe in an environment you do not trust to run code. Spec: `docs/specs/2026-09-02-merge-target-git-config.md`.
- Every refusal prints `REFUSED[<rail>]` naming which door stopped it - `pr-ref/foreign-host`, `passthrough/repo-flag`, `target/not-origin`, `env/git-config-injected`, and so on. Read the rail; it says what to change.
