...
- [ ] m2-boot-emitter-is-read-only — A full boot through bin/fm-boot-context.sh performs zero writes (red) **ready**
- [ ] m4-boot-budget-hostile — Boot stays inside its wall-clock budget with every helper wedged (red) **ready**
- [ ] m5-digest-never-silent — A section that fails to build says so; failure is never silent (red) **ready**
...
| m2-boot-emitter-is-read-only | red | A full boot through bin/fm-boot-context.sh performs zero writes |
| m4-boot-budget-hostile | red | Boot stays inside its wall-clock budget with every helper wedged |
| m5-digest-never-silent | red | A section that fails to build says so; failure is never silent |
