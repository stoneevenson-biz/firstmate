...
Two smaller properties fall out of the same discipline. A slug's segments may not be `.`, `..`, or lead with `-` — those pass a character-class check but are not GitHub names, and the contract says a malformed slug is refused rather than cleaned up. And when a remote's URL is printed raw (a `BADREMOTE` detail, or an `AMBIGUOUS` row for a candidate that is not a GitHub repository), its userinfo is redacted: the reader still sees which URL was no help, and a token embedded in a non-GitHub remote does not reach a banner.
