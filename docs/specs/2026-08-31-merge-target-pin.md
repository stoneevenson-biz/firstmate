...
**Passthrough is an allowlist.** Only `--merge`, `--squash`, `--rebase`, `--auto`, `--delete-branch`, and `--method`/`--body`/`--body-file`/`--subject` with a value reach `gh-axi pr merge`. Everything else is refused by name.

It began as a blocklist of repo flags, and that lost the same race twice. `gh` keeps the **last** repo flag it sees, so a passthrough one wins over the pin while stderr still announces the resolved target. The blocklist named `--repo x`, `--repo=x`, `-R x` and `-R=x` — and missed `-Rowner/repo`, the attached short form. Same defect, one spelling further along, and the next spelling would have been the next reject.

So the question is no longer *which flags are dangerous* — unknowable, since a host flag would redirect the whole API just as effectively — but *which flags this command actually takes*. That list is short and the binary documents it. A new merge flag costs one line; the alternative cost is a merge in a repository nobody named. The gate proves the refusal against a stub that honours the attached form and lets the last repo flag win, so it is a claim about the guard rather than about a stub too blind to see the smuggle.
