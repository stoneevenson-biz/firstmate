...
| `fm-verify.sh`           | The Quarterdeck gate between a crewmate's `done:` claim and firstmate's acceptance: adjudicates the worktree's gate ledger structurally ahead of both models, then the foreign lens over a patch built by `fm-patch-lib.sh` (refusing the lens outright when that patch is not valid), then a fresh-context default-REJECT verifier; records `approve:`/`reject:`/`escalate:`/`lens:` in `state/<id>.verdict`, which `fm-merge-local.sh` and `fm-pr-check.sh` require |
| `fm-patch-lib.sh`        | The one implementation of "what does the foreign lens actually read?": scopes the range to the branch's own base (furthest-forward among default-branch-shaped refs), spends its byte bound on whole FILES with every omission named in the payload and test material laid down first, and re-reads the finished artifact with `git apply --check` so a payload that is not a patch refuses the lens instead of being reviewed. Also a CLI (`bash bin/fm-patch-lib.sh <base\|build\|check> ...`) |
...
| `fm-watch-arm.sh`        | Verified per-home watcher re-arm; reports `started`, `healthy`, or `FAILED`; `--restart` relaunches only this home's watcher, and gates on the helm because it kills a live one. The kill needs positive proof: `fm_watch_lock_classify` must answer `ours-live`, a `stale` record is cleared without signalling anything, and a live pid that is `foreign` or `unidentified` is REFUSED out loud rather than signalled |
| `fm-watch.sh`            | Singleton-safe one-shot watcher; refuses to start at all if it cannot record a complete identity in its lock, because a watcher nobody can identify is a watcher nobody can stop; blocks until supervision work is due, queues it durably, then exits with one reason line. Senses staleness on the surface each `state/<id>.meta` records - `agent_status` from one `herdr api snapshot` per cycle, or the pre-cutover pane-text hash for a draining tmux window |
...
| `fm-wake-lib.sh`         | Shared durable wake queue and portable lock helpers sourced by the watcher, drain, arm, guard, and daemon; also owns the watcher lock's identity record (`fm_watch_lock_record_identity`, written and read back) and `fm_watch_lock_classify`, the one reading of that lock both the watcher and `--restart` act on |
| `fm-state-lib.sh`        | The one implementation of "which files in `state/` belong to task `<id>`?": the marker-key rule `fm-watch.sh` mints names with and `fm-teardown.sh` prunes them by, plus `fm_state_task_residue`, which SCANS for anything still named after a finished task rather than re-reading the prune list |
...
| `fm-teardown.sh`         | Return the worktree or retire/release a secondmate home and close its pane on the surface that created it, warning rather than staying silent when a pane could not be closed and keeping the task record at `state/<id>.orphan-pane` so that leftover pane stays findable (and clearing that record once a run does close the pane); protects ship work - accepting a squash merge only on a patch-id proof against the authoritative default branch, refreshing a stale remote-tracking ref once, and refusing when it cannot prove it - requires scout reports, checks child work, prunes every state file and watcher marker the task owned (`fm-state-lib.sh`) and warns on any residue, and prints the backlog reminder |
