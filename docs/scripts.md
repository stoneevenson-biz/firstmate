...
| `fm-patch-lib.sh`        | The one implementation of "what does the foreign lens actually read?": scopes the range to the branch's own base (furthest-forward among default-branch-shaped refs, and among remote-tracking refs only those whose remote this clone actually configures - a ref under `refs/remotes/` is not evidence that a remote exists, and the crewmate under review can write one), spends its byte bound - which covers the WHOLE artifact, header included - on whole FILES, with every omission named in the payload and test material laid down first, and re-reads the finished artifact with `git apply --check` so a payload that is not a patch refuses the lens instead of being reviewed. Also a CLI (`bash bin/fm-patch-lib.sh <base\|build\|check> ...`) |
