...
#
# THE BUG THIS REPLACES. `fm_mux_herdr_new_window` took a <session> argument and
...
# Creating is also exactly what `fm-herdr-workspaces.sh --apply` already does,
...
test_resolves_the_project_workspace_by_label() {
  reset
  local got
  got=$(FM_HERDR_WORKSPACE='' fm_mux_scope archify /p/archify)
...
test_resolution_survives_a_reshuffled_id_space() {
  reset
  local got
  got=$(HERDR_WORKSPACES="w1=firstmate,w2=archify" FM_HERDR_WORKSPACE='' fm_mux_scope archify /p/archify)
...
test_missing_workspace_is_created_for_the_project() {
  reset
  local got
  got=$(FM_HERDR_WORKSPACE='' fm_mux_scope brandnew /p/brandnew)
...
test_creation_failure_is_fatal_not_a_silent_fallback() {
  ...
  HERDR_WORKSPACES="" PATH="$(fm_mux_path_without_herdr)" fm_mux_scope nope /p/nope >/dev/null 2>&1 || rc=$?
...
test_override_by_label_wins() {
  reset
  local got
  got=$(FM_HERDR_WORKSPACE=config fm_mux_scope archify /p/archify)
...
test_override_by_raw_id_wins() {
  reset
  local got
  got=$(FM_HERDR_WORKSPACE=wM fm_mux_scope archify /p/archify)
...
test_override_naming_nothing_live_refuses() {
  reset
  local rc=0 err
  err=$(FM_HERDR_WORKSPACE=wDOESNOTEXIST fm_mux_scope archify /p/archify 2>&1) || rc=$?
...
test_new_window_passes_the_resolved_workspace() {
  reset
  local ws target
  ws=$(FM_HERDR_WORKSPACE='' fm_mux_scope archify /p/archify)
  target=$(fm_mux_new_window "$ws" fm-task-a7 /p/archify booking-fix)
...
test_an_empty_scope_cannot_produce_a_tab() {
  reset
  local rc=0 out
  out=$(fm_mux_new_window "" fm-task-a7 /p/archify booking-fix 2>&1) || rc=$?
