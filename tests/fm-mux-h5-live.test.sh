#!/usr/bin/env bash
# GATE h5 - a LIVE herdr pane, end to end through the seam.
#
...
#
#   scope -> new_window -> wait_ready -> run -> read -> cwd -> label -> close
#
...
cleanup() {
  [ -z "$TARGET" ] || fm_mux_close "$TARGET" 2>/dev/null || true
...
test_live_scope_creates_and_resolves_a_workspace() {
  WS=$(FM_HERDR_WORKSPACE='' fm_mux_scope "$WS_LABEL" "$WORKDIR") \
  ...
  local again
  again=$(FM_HERDR_WORKSPACE='' fm_mux_scope "$WS_LABEL" "$WORKDIR")
...
test_live_new_window_lands_in_that_workspace() {
  TARGET=$(fm_mux_new_window "$WS" fm-h5 "$WORKDIR" "gate-h5-probe") \
...
test_live_shell_readiness() {
  if ! FM_SHELL_READY_TIMEOUT=25 fm_mux_wait_ready "$TARGET" 25; then
...
test_live_cwd_tracks_the_pane() {
  ...
  assert_eq "$(fm_mux_cwd "$TARGET")" "$WORKDIR" "cwd did not report the pane's real directory"
  fm_mux_run "$TARGET" "cd '$sub'"
  got=""
  waited=0
  while [ "$waited" -lt 40 ]; do
    waited=$((waited + 1))
    got=$(fm_mux_cwd "$TARGET"); [ "$got" = "$sub" ] && break; sleep 0.25
  done
  assert_eq "$got" "$sub" "cwd did not follow a real chdir; the worktree wait would hang"
  fm_mux_run "$TARGET" "cd '$WORKDIR'"
...
# harness is available; set FM_MUX_LIVE_AGENT=0 to skip it deliberately.
test_live_send_is_actually_consumed() {
  if [ "${FM_MUX_LIVE_AGENT:-1}" = 0 ] || ! command -v claude >/dev/null 2>&1; then
    printf 'SKIP - the acknowledged-send case needs a real agent harness in the pane\n' >&2
    return 0
  fi
  local marker out rc=0 waited
  marker="H5ACK$$"
  fm_mux_run_launch "$TARGET" \
  ...
  while [ "$waited" -lt 90 ]; do
    ...
    if herdr agent get "$TARGET" 2>/dev/null | grep -q '"agent_status":"blocked"'; then
      fm_mux_send_key "$TARGET" enter
    fi
    if herdr agent get "$TARGET" 2>/dev/null | grep -qE '"agent_status":"(idle|done)"' \
       && fm_mux_read "$TARGET" 40 | grep -q 'bypass permissions'; then
      break
    fi
    sleep 1
  done
  ...
  fm_mux_send "$TARGET" "reply with exactly: $marker and nothing else" || rc=$?
  ...
  case "$rc" in
    0|4) : ;;
    *) fail "the live send failed outright (rc=$rc); pane: $(fm_mux_read "$TARGET" 12)" ;;
  esac
  out=""
  waited=0
  while [ "$waited" -lt 30 ]; do
    waited=$((waited + 1))
    out=$(fm_mux_read "$TARGET" 60)
