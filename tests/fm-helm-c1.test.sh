...
# and is refused only when it asks to spawn / steer / tear down / merge - at that
# verb, at that moment. This gate pins both halves, and the second half is the one
# firstmate got backwards twice: NO BOOT PATH MAY EMIT A REFUSAL. Several sessions
# on one home is the normal case, so anything that read as an error at boot would
...
run_verb() {  # <dir> <home> <args...> -> sets RUN_OUT / RUN_CODE
  local dir=$1 home=$2
  shift 2
  RUN_OUT=$(PATH="$dir/fakebin:$PATH" FM_HOME="$home" FM_ROOT_OVERRIDE="$dir/root" \
    "$@" 2>&1) && RUN_CODE=0 || RUN_CODE=$?
  return 0
}


test_drive_verbs_refuse_and_mutate_nothing() {
  local dir home pid before after verb label
  dir="$TMP_ROOT/refuse"
  mkdir -p "$dir"
  home=$(make_home "$dir")
  ...
  # One fingerprint over everything a drive verb could touch: the home (state,
  # data, project clone) and the FM_ROOT stand-in with its git dir.
  before=$(fm_helm_snapshot "$dir")

  while IFS='^' read -r label verb; do
    [ -n "$label" ] || continue
    # shellcheck disable=SC2086  # verb carries its own argument list
    run_verb "$dir" "$home" $verb
    [ "$RUN_CODE" -ne 0 ] || fail "$label was allowed through while another live session held the helm"
    assert_contains "$RUN_OUT" "refused: $label - another live session holds this home's helm (harness pid $pid)." \
      "$label did not refuse with the writer-only seam's message"
...
fm-spawn^$ROOT/bin/fm-spawn.sh t1 $home/projects/proj
fm-send^$ROOT/bin/fm-send.sh fm-t1 hello
fm-teardown^$ROOT/bin/fm-teardown.sh t1
fm-merge-local^$ROOT/bin/fm-merge-local.sh t1
fm-pr-check^$ROOT/bin/fm-pr-check.sh t1 https://example.invalid/pr/1
fm-promote^$ROOT/bin/fm-promote.sh t1
fm-update^$ROOT/bin/fm-update.sh
ROWS

  after=$(fm_helm_snapshot "$dir")
  if [ "$before" != "$after" ]; then
    fail "a refused writer mutated the tree"$'\n'"$(diff <(printf '%s\n' "$before") <(printf '%s\n' "$after") || true)"
  fi
...
# fingerprint is watching: a state write, and a git fast-forward.
test_positive_controls() {
  local dir home before_head after_head
  ...
  pass "with the helm free the same calls DO mutate - the refusal assertions are not vacuous"
}
...
test_no_boot_path_refuses() {
  ...
  # (How the emitter words the observer line is its own gate's business, not this
  # one's; asserting on that wording here would only make this gate flaky.)
...
SH
  ...
  pass "no boot path emits a refusal under a live foreign helm - emitter, lock status, guard, drain, bootstrap, install"
}
...
test_seam_roster_is_writer_only() {
  ...
  [ "$callers" = "fm-merge-local.sh fm-pr-check.sh fm-promote.sh fm-send.sh fm-spawn.sh fm-teardown.sh fm-update.sh " ] \
  ...
  for f in fm-bootstrap.sh fm-boot-context.sh fm-captain-bootstrap.sh fm-guard.sh \
           fm-wake-drain.sh fm-watch.sh fm-watch-arm.sh fm-peek.sh fm-status.sh \
  ...
  pass "only the seven drive verbs gate; every boot and read path is clear of the seam"
}


# EDGE POLICY: harness-not-found FAILS OPEN, out loud. fm_lock_harness_pid can
# fail to identify a harness at all - an unverified adapter, a wrapper, a shell
# outside any agent. Failing closed there would be a lock-out by another route:
# such a session could never prove it is the holder, so it could never drive, and
# --take would not help because the fault is not with the holder. Exercised at the
# function, because the ancestry a test runs under is not something it can choose.
# shellcheck disable=SC2031  # the subshells here only OVERRIDE fm_lock_harness_pid;
# nothing they set is read back, so the "modified in a subshell" warning is noise.
test_require_helm_policy_matrix() {
  local dir state pid out rc
  dir="$TMP_ROOT/matrix"
  state="$dir/state"
  mkdir -p "$state"
  pid=$(fm_helm_live_harness "$dir" claude)

  # free
  rc=0; out=$(. "$ROOT/bin/fm-lock-lib.sh"; fm_lock_require_helm "$state" probe 2>&1) || rc=$?
  expect_code 0 "$rc" "a free lock must never refuse a writer"
  [ -z "$out" ] || fail "a free lock must be silent, got: $out"

  # stale: a recorded holder that is dead is not a steering session
  printf '99999999\n' > "$state/.lock"
  rc=0; out=$(. "$ROOT/bin/fm-lock-lib.sh"; fm_lock_require_helm "$state" probe 2>&1) || rc=$?
  ...
  # harness-not-found: allowed through, and NOT silently
  rc=0; out=$(. "$ROOT/bin/fm-lock-lib.sh"
    fm_lock_harness_pid() { return 1; }
    fm_lock_require_helm "$state" probe 2>&1) || rc=$?
  expect_code 0 "$rc" "an unidentifiable harness must not be blocked - that is a lock-out by another route"
  assert_contains "$out" "cannot identify this session's harness" \
  ...
  pass "helm policy matrix: free, stale and ours pass; theirs refuses; harness-not-found fails open, out loud"
}
