...
reset_record
out=$("$MERGE" t1e 5 --project "$BOTH" --repo other-owner/other-repo 2>&1); code=$?
...
# --- 6. a non-origin target merges, but never silently ----------------------
reset_record
out=$("$MERGE" t1f "https://github.com/$UPSTREAM_SLUG/pull/5" --project "$BOTH" 2>&1); code=$?
expect_code 0 "$code" "a named upstream target is legitimate and must merge: $out"
...
pass "non-origin target: merges because it was named, and says so loudly"
